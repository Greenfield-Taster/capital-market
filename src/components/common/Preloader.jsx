...
// Створюємо контекст для стану прелоадера
...
const Preloader = () => {
  ...
  // Ефект для головного прелоадера (при початковому завантаженні сайту)
  useEffect(() => {
    if (!initialLoadComplete) {
      // Використовуємо більший період часу (3с) для демонстрації повної анімації SVG
      ...
      // Якщо сторінка вже була завантажена один раз, не показуємо головний прелоадер
      setLoading(false);
    }
  }, [initialLoadComplete, setInitialLoadComplete]);

  // Ефект для прелоадера навігації (між сторінками)
  useEffect(() => {
    // Показуємо прелоадер навігації тільки при зміні шляху і після першого завантаження
    if (initialLoadComplete) {
      setNavLoading(true);
      const navTimer = setTimeout(() => {
        setNavLoading(false);
      }, 800); // Короткий час для навігації - 800 мс
